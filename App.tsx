...
const ExportModal: React.FC<{ 
  ...
  destination: string;
}> = ({ isOpen, onClose, onExportPDF, onExportPNG, onExportFile, isExportingPNG, destination }) => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: `ARKIV - ${destination}`,
          text: `Check out my travel archive for ${destination}!`,
          url: shareUrl,
        });
      } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-[5000] flex items-center justify-center p-4 bg-black/20 backdrop-blur-md">
      <div className="w-full max-w-[280px] bg-white rounded-[1.5rem] border-[1px] border-[#fbcfe8] p-6 shadow-2xl animate-float">
        <div className="flex justify-between items-center mb-5">
          <h3 className="text-xl font-retro text-orange-500 uppercase italic">EXPORT_HUB</h3>
          <button onClick={onClose} className="text-slate-300 hover:text-orange-500 transition-colors"><X size={20} /></button>
        </div>
        
        <div className="grid grid-cols-1 gap-3">
          <button 
            onClick={() => { onExportPDF(); onClose(); }}
            className="flex items-center gap-4 p-3 bg-orange-50 hover:bg-orange-100 rounded-xl border border-white transition-all group"
          >
            <div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center text-orange-400 shadow-sm border border-orange-100 group-hover:scale-110 transition-transform">
              <Download size={20} />
            </div>
            <div className="text-left">
              <div className="text-[10px] font-black uppercase text-orange-600 tracking-wider">Save as PDF</div>
              <div className="text-[7px] font-digital opacity-60 uppercase">Print ready layout</div>
            </div>
          </button>
          ...
          <button 
            ...
            className="flex items-center gap-4 p-3 bg-pink-50 hover:bg-pink-100 rounded-xl border border-white transition-all group disabled:opacity-50"
          >
            <div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center text-pink-400 shadow-sm border border-pink-100 group-hover:scale-110 transition-transform">
              {isExportingPNG ? <Loader2 size={20} className="animate-spin" /> : <ImageIcon size={20} />}
            </div>
            <div className="text-left">
              <div className="text-[10px] font-black uppercase text-pink-600 tracking-wider">Save Image</div>
              <div className="text-[7px] font-digital opacity-60 uppercase">Full gallery snapshot</div>
            </div>
          </button>

          <button 
            onClick={() => { onExportFile(); onClose(); }}
            className="flex items-center gap-4 p-3 bg-slate-50 hover:bg-slate-100 rounded-xl border border-white transition-all group"
          >
            <div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center text-slate-400 shadow-sm border border-slate-100 group-hover:scale-110 transition-transform">
              <FileBox size={20} />
            </div>
            <div className="text-left">
              <div className="text-[10px] font-black uppercase text-slate-600 tracking-wider">Save File</div>
              <div className="text-[7px] font-digital opacity-60 uppercase">Local .arkiv database</div>
            </div>
          </button>
          ...
          <button 
            onClick={handleShare}
            className="flex items-center gap-4 p-3 bg-green-50 hover:bg-green-100 rounded-xl border border-white transition-all group"
          >
            <div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center text-green-400 shadow-sm border border-green-100 group-hover:scale-110 transition-transform">
              <Share2 size={20} />
            </div>
            <div className="text-left">
              <div className="text-[10px] font-black uppercase text-green-600 tracking-wider">Share Link</div>
              <div className="text-[7px] font-digital opacity-60 uppercase">Invite collaborators</div>
            </div>
          </button>
        </div>
      </div>
...
const InviteModal: React.FC<{ isOpen: boolean; onClose: () => void; tripId: string }> = ({ isOpen, onClose, tripId }) => {
  ...
  return (
    <div className="fixed inset-0 z-[5000] flex items-center justify-center p-4 bg-green-900/5 backdrop-blur-md">
      <div className="w-full max-w-[260px] bg-white rounded-[1.5rem] border-[1px] border-[#fbcfe8] p-5 shadow-xl animate-float text-center">
        <div className="flex justify-center mb-3">
          <div className="w-10 h-10 rounded-[1rem] bg-gradient-to-br from-green-400 to-yellow-300 border border-white flex items-center justify-center text-white">
            <Users size={20} />
          </div>
        </div>
        ...
        <div className="bg-pink-50 p-3 rounded-[1rem] border border-white mb-4">
          <label className="text-[7px] font-digital text-pink-300 uppercase tracking-widest block mb-1 text-left">LINK</label>
          <div className="flex gap-2">
            <input 
              readOnly 
              className="bg-white border border-[#fbcfe8] flex-1 py-1 px-3 rounded-full text-[8px] font-digital outline-none truncate" 
              value={shareUrl} 
            />
            <button 
              onClick={handleCopy}
        ...
        <button 
          onClick={onClose}
          className="w-full bg-green-500 text-white font-black py-2 rounded-full text-[8px] tracking-widest uppercase hover:bg-green-600 transition-all border border-white shadow-lg"
        >
          CLOSE
        </button>
      </div>
...
const AddPlaceModal: React.FC<{ isOpen: boolean; onClose: () => void; onAdd: (data: Partial<Place>) => void }> = ({ isOpen, onClose, onAdd }) => {
  ...
  return (
    <div className="fixed inset-0 z-[3000] flex items-center justify-center p-4 bg-green-900/5 backdrop-blur-sm">
      <div className="w-full max-w-[320px] bg-white rounded-[1.5rem] border-[1px] border-[#fbcfe8] p-5 shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <input 
            autoFocus
            className="w-full bg-green-50/50 border border-[#fbcfe8] py-3 px-5 rounded-xl font-korean text-green-900 outline-none focus:border-green-400 text-sm shadow-inner"
            placeholder="Place name or URL..."
            value={input}
            onChange={e => setInput(e.target.value)}
          />
          <button type="submit" disabled={loading} className="w-full bg-green-500 text-white font-black py-3 rounded-full flex items-center justify-center gap-2 hover:bg-green-600 transition-all border border-white shadow-lg text-[11px] uppercase tracking-widest">
...
const ReceiptModal: React.FC<{ isOpen: boolean; onClose: () => void; places: Place[] }> = ({ isOpen, onClose, places }) => {
  ...
  return (
    <div className="fixed inset-0 z-[4000] flex items-center justify-center p-4 bg-black/30 backdrop-blur-lg overflow-y-auto">
      <div className="w-full max-w-[340px] bg-white rounded-t-[1.5rem] border-x-[1px] border-t-[1px] border-[#fbcfe8] relative animate-in fade-in slide-in-from-bottom-4 duration-500 shadow-xl">
        ...
        <div className="p-6 pt-10 font-digital text-slate-700">
          <div className="text-center mb-5 border-b border-dashed border-slate-200 pb-3">
          ...
          <div className="border-t-2 border-double border-slate-200 pt-3 flex justify-between items-end">
             <div className="flex flex-col">
...
export default function App() {
  ...
  const generateAI = async (day: number) => {
    ...
    try {
      ...
      handleAddPlace(day, { name: "New Entry" });
    } finally {
  ...
  const handleExportPNG = async () => {
    ...
    try {
      ...
      link.download = `${meta?.destination || 'trip'}_archive.png`;
      link.href = dataUrl;
      link.click();
    } catch (err) {
  ...
  const handleExportFile = () => {
    const data = JSON.stringify({ meta, places }, null, 2);
    const blob = new Blob([data], { type: 'application/octet-stream' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${meta?.destination || 'trip'}_archive.arkiv`;
    a.click();
  };

  if (!tripId || !meta) {
    return (
      <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden bg-white select-none">
        ...
        <div className="w-full max-w-xl flex flex-col items-center gap-4 z-20">
          ...
          <div className="flex flex-col md:flex-row items-center gap-5 w-full max-md px-4">
             <div className="w-20 h-20 rounded-[1.2rem] bg-gradient-to-br from-[#bef264] to-[#f59e0b] flex items-center justify-center text-white animate-float shadow-lg flex-shrink-0 border-[2px] border-white">
                ...
                <form onSubmit={e => { e.preventDefault(); handleLaunch(); }} className="flex flex-col gap-2.5 w-full">
                  <div className="relative">
                    <input 
                      ...
                      className="w-full bg-white border-[1px] border-[#fbcfe8] py-2.5 px-6 rounded-full text-green-900 text-sm font-retro italic placeholder:text-[#bbf7d0] shadow-md focus:shadow-green-50 focus:outline-none transition-all placeholder:not-italic font-korean" 
                      value={tempDest} 
                      onChange={e => setTempDest(e.target.value)} 
                    />
                  </div>
                  
                  <button type="submit" className="bg-[#86efac] hover:bg-[#4ade80] text-white font-black px-6 py-2.5 rounded-full text-sm uppercase transition-all shadow-md active:scale-95 border border-white flex items-center justify-center gap-2 w-max group">
  ...
  return (
    <div className="min-h-screen flex flex-col font-bubbly relative planning-bg">
      <header className="px-5 pt-5 pb-2 flex items-center justify-between sticky top-0 z-[100] glass-light print-hide">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setIsInviteOpen(true)} className="px-3 py-1.5 bg-pink-400 text-white rounded-full border border-white hover:scale-105 transition-all text-[9px] font-black tracking-widest uppercase flex items-center gap-1.5 shadow-md">
            <Users size={12} /> COLLAB
          </button>
          <button onClick={() => setIsReceiptOpen(true)} className="px-3 py-1.5 bg-green-500 text-white rounded-full border border-white hover:scale-105 transition-all text-[9px] font-black tracking-widest uppercase flex items-center gap-1.5 shadow-md">
            <Receipt size={12} /> SETTLE
          </button>
          <button onClick={() => setIsExportOpen(true)} className="bg-orange-400 text-white font-black px-4 py-1.5 rounded-full text-[9px] tracking-widest uppercase hover:scale-105 transition-all shadow-md border border-white flex items-center gap-1.5">
            <Download size={12} /> EXPORT
          </button>
        </div>
      </header>

      <main ref={mainRef} className="flex-1 overflow-x-auto custom-scrollbar px-5 py-6 relative z-10 flex h-full items-start gap-4 print:block">
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
          ...
          <button onClick={() => setMeta(p => p ? { ...p, duration: p.duration + 1 } : null)} className="flex-none w-10 h-[200px] bg-white/40 backdrop-blur-sm rounded-[1rem] border-[1px] border-dashed border-[#fbcfe8] hover:border-green-400 flex items-center justify-center text-pink-300 hover:text-green-500 transition-all shadow-sm print-hide mt-8"><Plus size={24} className="rotate-90" /></button>
        </DndContext>
      </main>
      ...
      <footer className="px-6 py-1.5 border-t border-[#fbcfe8] bg-white/40 backdrop-blur-md flex items-center justify-between z-50 print-hide">
         <div className="flex items-center gap-4">
            <span className="text-[8px] font-digital text-green-600 uppercase tracking-[0.1em] font-black">SYNC</span>
            <div className="w-32 h-1.5 bg-white rounded-full overflow-hidden border border-[#fbcfe8] shadow-inner">
         ...
         <div className="flex items-center gap-4">
            {tripId && (
              <div className="flex items-center gap-2 bg-pink-100/60 px-2 py-0.5 rounded-full border border-white shadow-sm">
                <div className="w-1 h-1 bg-green-500 rounded-full animate-pulse" />
                <span className="text-[8px] font-black uppercase text-pink-500">Live</span>
              </div>
            )}
            <span className="text-[8px] font-black tracking-[0.1em] uppercase text-pink-400">ARKIV v11</span>
         </div>
      </footer>
    </div>
  );
